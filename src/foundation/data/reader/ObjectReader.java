...
public class ObjectReader extends EntityReader implements Iterable<PropertyReader> {
	...
	private Set<String> excludePropertys = null;
		...
		while (dataType != null) {
			...
			for (Method method: methods) {
				...
				if (methodName.startsWith("get")) {
					...
					if (EnvelopWriter.Name_ExcludeProperty.equalsIgnoreCase(methodName)) {
						readExcludePropertys(method);
						continue;
					}
	...
	private void readExcludePropertys(Method excludePropertysMethod) {
		try {
			excludePropertys = (Set<String>)excludePropertysMethod.invoke(null);
		}
		catch(Exception e) {
			logger.error("error get excludePropertys: " + excludePropertysMethod.getName());
		}
	}
	...
	public String getJSONString(Object entity) throws Exception {
		...
		for (int i = 0; i < size; i++) {
			...
			if (excludePropertys != null && excludePropertys.contains(name)) {
				continue;
			}
