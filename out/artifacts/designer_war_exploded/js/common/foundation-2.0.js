...
function convertTime(time) {
	...
	if(now.getYear() > createDate.getYear()) {
		return  (now.getYear()- createDate.getYear()) +"年前";
	}
	else  if(now.getMonth() > createDate.getMonth()) {
		return  (now.getMonth()- createDate.getMonth()) +"月前";
	}		
	else  if(now.getDate() > createDate.getDate()) {
		return  (now.getDate()- createDate.getDate()) +"天前";
	}		
	else  if(now.getHours() > createDate.getHours()) {
		return  (now.getHours()- createDate.getHours()) +"小时前";
	}		
	else  if(now.getMinutes() >= createDate.getMinutes()) {
		return  (now.getMinutes()- createDate.getMinutes()) +"分钟前";
	}		
}
...
(function ($) {
	...
	var ServerClass = Object.subClass({
		...
		ajaxRequest: function(url, param, callback, name, onSuccess, onFail) {
			var me = this;
			if (param) {
				if ($.isFunction(param)) {
					callback = param;
			...
			var callbackname = "successjsonpCallback";
			if(name) {
				callbackname = name;
		...
		getSysParam: function(name, callBack) {
			...
			if (aServer.loaded) {
				...
				me.ajaxRequest(url, null, callback, name,  function(doCallback, result) {
					if (doCallback) {
		...
		saveData: function(url, data, callback) {
			...
			me.ajaxRequest(url, param, callback, name, function(doCallback, result) {
				if (doCallback) {
		...
		getData: function(url, param, name, callback) {
			if (!url) {return;} var me = this;
		...
		call: function(url, data, callback, name) {
			if (!url) {return;} var me = this;
			...
			me.ajaxRequest(url, data, callback, name, afterRequest, afterRequest);
		},
	...
	$.fm.WinClass = Control.subClass({
		template: [
		    '<div align="center" class="win_mask">',
